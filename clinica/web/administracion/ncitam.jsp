                        ...
                        <div class="col-lg-12">
                            <div class="form-panel">
                                <form  id="listarei" name="listarei" action="" method="POST">
                                    ...
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(idm);%>" required class="form-control" id="especialista" name="especialista" placeholder="Especialista">
                                    </div>
                                    <% ResultSet v = con.consulta("select * from personas where dni='" + idm + "'");
                                        while (v.next()) {
                                    %>
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(v.getString("nombres")); %>" required class="form-control" id="nespecialista" name="nespecialista" placeholder="Especialista">
                                    </div>
                                    <%}%>
                                    ...
                                    %>
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(v.getString("nomesp")); %>" required class="form-control"  name="especial" placeholder="Paciente">
                                    </div>
                                    <%}%>
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(dni);%>" required class="form-control" id="paciente" name="paciente" placeholder="Paciente">
                                    </div>
                                    <%  v = con.consulta("select * from personas where dni='" + dni + "'");
                                        while (v.next()) {
                                    %>
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(v.getString("nombres")); %>" required class="form-control"  name="npaciente" placeholder="Paciente">
                                    </div>
                                    <%}%>

                                    <div class="form-group">
                                        <input type="text" value="<%out.print(fecha);%>" required class="form-control" id="fecha" name="fecha" placeholder="Fecha">
                                    </div>
                                    <div class="form-group">
                                        <input type="text" value="<%out.print(hora);%>" required class="form-control" id="hora" name="hora" placeholder="Hora">
                                    </div>
                                ...
                                <%
                                    if (request.getParameter("enviar") != null) {
                                        ...
                                        if (contador.equals("0")) {
                                            con.consulta("INSERT INTO citas( asunto, fecha, hora, medico, paciente,estcita,estpago,especialidad) VALUES ('" + asunto + "', '" + fecha + "','" + hora + "', '" + idm + "', '" + dni + "', 'Pendiente','Pendiente', '" + especial + "');");
                                            MailSender ms = new MailSender();
                                            ms.mensaje("http://localhost:8080/clinica/administracionp/proforma.jsp?hora=" + hora + "&&fecha=" + fecha + "&&idd=" + idm + "");
                                            ms.para(user);
                                            ms.SendMail();
                                            out.println("<script language='javascript'>window.location='../administracion/listaresp.jsp?id=" + dni + "'</script>;");
                                            out.print("<script>alert('Registro Guardado');</script>");
                                        } else {
                                            out.print("<script>alert('Esta Fecha y hora ya esta Registrada');</script>");
                                        }
