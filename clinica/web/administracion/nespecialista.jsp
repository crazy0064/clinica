                    ...
                    <div class="row mt">
                        ...
                        <div class="col-lg-12">
                            <div class="form-panel">
                                <form  id="nespecialista" name="nespecialista" action="nespecialista.jsp?id=<%out.print(id);%>" method="POST">
                                    <%rs = con.consulta("select * from personas where dni='" + id + "'");
                                        while (rs.next()) {%>
                                    <div class="form-group">
                                        <input type="text" disabled class="form-control" value="<%out.print(rs.getString("dni"));%>" id="dni" name="dni" placeholder="Ingrese su numero DNI de ciudadano" onkeydown="return numeros(event)" onchange="evaluar(registro)">
                                    </div>
                                    <div class="form-group">
                                        <input type="text" disabled class="form-control" value="<%out.print(rs.getString("nombres"));%>" id="nombres" name="nombres" placeholder="Ingrese sus nombres">
                                    </div>
                                    <div class="form-group">
                                        <input type="text" disabled class="form-control" value="<%out.print(rs.getString("apellidos"));%>" id="apellidos" name="apellidos" placeholder="Ingrese sus apellidos">
                                    </div>
                                    <div class="form-group">
                                        <div class="form-group">
                                            <select name="especial" required class="form-control" id="especial">
                                                <option value="">Seleccione la especialidad</option>
                                                <%
                                                    ResultSet rs1 = con.consulta("select * from especialidad where estesp='true'");
                                                    while (rs1.next()) {
                                                        out.print("<option value='" + rs1.getString("codesp") + "'>" + rs1.getString("nomesp") + "</option>");
                                                    }
                                                %>
                                    ...
                                    <div class="form-group">
                                        <input type="email" disabled value="<%out.print(rs.getString("correo"));%>" class="form-control" id="correo" name="correo" placeholder="Ingrese su correo electronico" onchange="validare(registro);">
                                    </div>
                                    <%}%>
                                    <button type="submit" class="btn btn-default btn-lg btn-block" name="enviar" id="enviar">Guardar Registro</button>
                                </form>
                                <%
                                    if (request.getParameter("enviar") != null) {

                                        String especial = request.getParameter("especial");
                                        rs = con.consulta("select count(*) from doctores where  espdoctor='" + especial + "' and dnidoctor='" + id + "'");
                                        while (rs.next()) {
                                            if (rs.getInt("count") == 0) {
                                                con.consulta("INSERT INTO doctores(dnidoctor, espdoctor)VALUES ('" + id + "', '" + especial + "');");
                                                out.print("<script>alert('Registro Guardado');</script>");
                                                out.println("<script language='javascript'>window.location='../administracion/listaresp.jsp?id=" + id + "'</script>;");
                                            } else {
                                                out.print("<script>alert('Especialidad ya registrada');</script>");
                                            }
                                        }
