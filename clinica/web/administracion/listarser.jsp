                    ...
                    <div class="row mt">
                        ...
                        <div class="col-lg-12">
                            <div class="form-panel">
                                <form  id="listaser" name="listarser" action="listarser.jsp" method="POST">
                                    <div class="form-group">
                                        <input name="codigoser" type="text" required class="form-control" id="codigoser" placeholder="Ingrese codigo unico de Especialidad" title="Ingrese codigo unico de Especialidad" maxlength="3">
                                    </div>
                                    <div class="form-group">
                                        <input type="text" required class="form-control" id="nombreser" name="nombreser" placeholder="Ingrese el nombre del servicio" title="Ingrese el nombre del servicio">
                                    </div>
                                    <button type="submit" class="btn btn-default btn-green" id="enviar" name="enviar">Registrar Nuevo Servicio</button>
                                </form>
                                <%
                                    if (request.getParameter("enviar") != null) {
                                        String codigo = request.getParameter("codigoser");
                                        String nombre = request.getParameter("nombreser");
                                        rs = con.consulta("select count(*) from servicios where idservicio='" + codigo + "'");
                                        String contador = "";
                                        while (rs.next()) {
                                            contador = rs.getString("count");
                                        }
                                        if (contador.equals("0")) {
                                            con.consulta("INSERT INTO servicios(nomservicio, idservicio)VALUES ('" + nombre + "', '" + codigo + "');");
                                        } else {
                                            out.print("<script>alert('El codigo o Especialidad Existen');</script>");
                                        }
                                ...
                                <br>
                                <form  id="listarser" name="listarser" action="listarser.jsp" method="POST">
                                    <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <% String consulta = "select * from servicios limit 100";

                                        if (request.getParameter("buscar") != null) {
                                            String valor = request.getParameter("tbuscar");
                                            String buscar = request.getParameter("busqueda");
                                            if (valor.equals("0")) {
                                                consulta = "select * from servicios where idservicio like'%" + buscar + "%'";
                                            } else if (valor.equals("1")) {
                                                consulta = "select * from servicios where nomservicio like'%" + buscar + "%'";
                                            }
                                        }
                                        ...
                                        while (rs.next()) {
                                            out.print("   <tr>");
                                            out.print("  <td class='hidden-phone'>" + rs.getString("idservicio") + "</td>");
                                            out.print("    <td >" + rs.getString("nomservicio") + "</td>");
                                            if (rs.getString("estservicio").equals("t")) {
                                            ...
                                            out.print("    <td>");
                                            out.print("  <a href = '../administracion/mservicio.jsp?id=" + rs.getString("idservicio") + "' <button class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></button></button> </a>");
                                            out.print("  <a href = '../procesos/aservicio.jsp?id=" + rs.getString("idservicio") + "' <button class='btn btn-success btn-xs'><i class='fa fa-play'></i></button></a>");
                                            out.print("   <a href = '../procesos/eservicio.jsp?id=" + rs.getString("idservicio") + "' <button class='btn btn-warning btn-xs'><i class='fa fa-stop'></i></button></a>");
                                            out.print("   <a href = '../procesos/elservicio.jsp?id=" + rs.getString("idservicio") + "' <button class='btn btn-danger btn-xs'><i class='fa fa-eraser'></i></button></a>");
                                            out.print("   </td>");
                                            out.print(" </tr>");
                                        }
                                        out.print("  </tbody>");
